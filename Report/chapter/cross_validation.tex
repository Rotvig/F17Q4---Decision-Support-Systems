...
\end{lstlisting}
\todo{Remove anything but the last line of the listing? And dont explain the ? rows, because its not really relevant?}
...
%The Mean Squared Error is printed as in Listing \ref{lst:poly_print_valid}. It shows the previously stated mse of 24.42. Furthermore the MSE for when using a second degree polynomial is 19.46 and 19.55 for third degree polynomial.

% and the result is shown in Figure \ref{fig:result_valid}.

%\myFigure{validationSetApproach_result.png}{Result of Validation Set Approach}{fig:result_valid}{0.4}
\todo{Har fjernet meget her som jeg bare synes bliver opsummeret i det følgende afsnit}
...
\myFigure{validationSetApproach_poly.png}{Validation error estimates for a single split}{fig:poly_valid}{0.5} \todo{Needs to be removed if we can't use figures from the book. Tænker vi bare lige plotter i python}


\section{Leave-one-out cross validation}
%Leave-one-out Cross-validation, or \emph{LOOCV}, is closely related to the Validation Set Approach specified in Section \ref{sec:VSA}. The thought process is the same, as you split the data into two parts, a training set and a test set. However, where Validation Set Approach split the data into two subsets of comparable size, the LOOCV uses a single observation as the test set, and the remaining observations are used as the training set. It then iterates through using each observations as test set and the others as training set. This is illustrated in Figure \ref{fig:loocv}. 
...
\end{lstlisting}
\FloatBarrier
%In Figure \ref{fig:result_K_fold} the result of the K-fold Cross-validation is shown from 1 to 10 degree polynomial. 
...
\FloatBarrier
%The Mean Squared Error is higher than the result of both the Validation Set Approach, shown in Figure \ref{fig:result_valid}, and the Leave-one-out Cross-validation, shown in Figure \ref{fig:result_LOOCV}, but shows the same tendency to oscillate after the initial improvement. 
...
%\myFigure{K_fold_result.png}{Result of K-fold Cross-validation}{fig:result_K_fold}{0.25}
