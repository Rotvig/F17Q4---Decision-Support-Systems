...
When plotting the dendrograms The only thing that differ is which linkage object that are send as a parameter to the dendrogram function.
...
But looking at figures \ref{fig:denComplete} and \ref{fig:denAverage} Average and Complete looks more identical and correct compared to singles figure in figure \ref{fig:dendrogramsingle}. In the dendrogram for single it can be seen that one observation belongs to its own cluster. This can mean that maybe two clusters are not the best solution, and the data set should maybe be divided into more clusters. In this case four cluster would be better.