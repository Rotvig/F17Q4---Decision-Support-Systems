...
\myFigure{bootstrap_example.PNG}{A graphical example of the bootstrap approach}{fig:bootstrap}{0.6} \todo{Vi skal vel lave vores egne figurer?}
...
%Det er beskrevet lige oven over. 
...
First of all a function for calculating $\alpha$ is required. The function is shown in listing \ref{lst:alpha}. The function receives \emph{data} which is a vector with the properties X and Y. The second input is \emph{index} which is used to access the wanted X and Y value in the vector \emph{data}.
...
\todo{Viser vi resultater på denne måde? Det er jo ikke en udregning}
\begin{equation}
\hat{\alpha} = 0.5834
\end{equation}
...
%  Er det relevant om det passer med resultatet i bogen? Vi skal vel mere bare forklare hvad resultatet betyder. fx som jeg har skrevet her
The Standard Diviation of the estimates is roughly 0.09. This means that when looking at a random sample of the population data, the difference between $\hat{\alpha}$ and $\alpha$ is expected to be roughly 0.9 on average.
...
\todo{samme som ovenfor med resultater}
\begin{equation}
